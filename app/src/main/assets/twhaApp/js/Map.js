...
function Map() {
  ...
  document.addEventListener('touchstart', function (e) {
    mousedown_x = e.changedTouches[0].pageX;
    mousedown_y = e.changedTouches[0].pageY;
    console.log(mousedown_x);
    console.log(mousedown_y);
    // e.preventDefault();
  });
  document.addEventListener('touchmove', function (e) {
    if (e.buttons != 0 && (mousedown_x !== e.pageX || mousedown_y !== e.pageY)) {
      data.map_x += mousedown_x - e.changedTouches[0].pageX;
      data.map_y += mousedown_y - e.changedTouches[0].pageY;
      limit_map_center();
      mousedown_x = e.changedTouches[0].pageX;
      mousedown_y = e.changedTouches[0].pageY;
      update_map();
  ...
  document.addEventListener('keydown', function (event) {
    ...
    if (!event.shiftKey) {
      if (keyName === 'ArrowRight') {
        ...
        update_map();
        update_info();
        console.log(data.map_x);
        console.log(data.map_y);
